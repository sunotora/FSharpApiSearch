...
module FSharpSignatureParser =
  ...
  let partialName = sepBy1 name (pchar '.') |>> (List.map (fun n -> { DisplayName = n; GenericParameterCount = 0 } ) >> List.rev)
    ...
    let id =
      ...
      | Identity (PartialIdentity p) ->
        let newName =
          match p.Name with
          | [] -> []
          | n :: tail -> { n with GenericParameterCount = parameterCount } :: tail
  ...
  let arraySymbol = regex arrayRegexPattern |> trim |>> (fun array -> Identity (PartialIdentity { Name = [ { DisplayName = array; GenericParameterCount = 1 } ]; GenericParameterCount = 1 }))
  let maybeArray t =
    t
    .>>. many arraySymbol
    |>> (function
